...
int res_unpack(vorbis_info_residue *info,
  ...
  info->type=tremoroggpack_read(opb,16);
  if(info->type>2 || info->type<0)goto errout;
  info->begin=tremoroggpack_read(opb,24);
  info->end=tremoroggpack_read(opb,24);
  info->grouping=tremoroggpack_read(opb,24)+1;
  info->partitions=tremoroggpack_read(opb,6)+1;
  info->groupbook=tremoroggpack_read(opb,8);
  ...
  for(j=0;j<info->partitions;j++){
    int cascade=tremoroggpack_read(opb,3);
    if(tremoroggpack_read(opb,1))
      cascade|=(tremoroggpack_read(opb,5)<<3);
  ...
  for(j=0;j<info->partitions;j++){
	...
	unsigned char book=tremoroggpack_read(opb,8);
