...
visiting the vertexs 'descendents' (as done in a normal depth-first search),
...
int lprofP_callhookOUT(lprofP_STATE* S) {
	lprofS_STACK_RECORD *caller;
	lprofS_STACK_RECORD dummy;
	...
	if (!caller) {
		...
		dummy.file_defined = "-";
		dummy.function_name = "-";
		dummy.line_defined = 0;
		dummy.current_line = 0;
	}
	...
	info->total_time += function_call_time;
        ...
        formats(caller->file_defined);
        formats(caller->function_name);
        output("%d\t%s\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%f\t%f\n",
	       S->stack_level,
	       info->file_defined,
	       info->function_name, 
	       info->line_defined,
...
lprofP_STATE* lprofP_init_core_profiler(const char *_out_filename, int isto_printheader, float _function_call_time) {
	...
	if (isto_printheader) {
		output("stack_level\tfile_defined\tfunction_name\tline_defined\tcurrent_line\ttcaller_file_defined\tcaller_function_name\tcaller_line_defined\tcaller_current_line\tlocal_time\ttotal_time\n");
	}
