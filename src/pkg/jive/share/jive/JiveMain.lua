...
function Menu:addItem(item, weight)
	...
	for i=1,menu:numItems() do
		...
		if weight == j.weight and tostring(item:getValue()) < tostring(j:getValue()) then
			self.menu:insertItem(item, i)
			return
		end
	end
