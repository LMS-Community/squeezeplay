...
function classFunction:supcode (local_constructor)
 ...
 local out = string.find(self.mod, "tolua_outside")
	 or string.find(self.mod, "tolua_create")
	 or string.find(self.mod, "tolua_destroy")
 -- call function
 ...
 elseif class and self.name == 'operator&[]' then
  ...
  if self.type ~= '' and self.type ~= 'void' then
   ...
   if t then
    ...
    elseif string.find(self.mod, "tolua_create") then
...
function _Function (t)
 ...
 append(t)
 if t:inclass() then
-- print ('t.name is '..t.name..', parent.name is '..t.parent.name..' t.mod is '..t.mod)
  ...
  elseif string.gsub(t.name, "%b<>", "") == '~'..string.gsub(t.parent.original_name or t.parent.name, "%b<>", "") then
   t.name = 'delete'
   t.lname = 'delete'
   t.parent._delete = "tolua_collect_" .. clean_template(self.type)
  end
 end
 t.cname = t:cfuncname("tolua")..t:overload(t)

 if t.mod == 'tolua_destroy' then
  t.parent._delete = t.cname
 end
